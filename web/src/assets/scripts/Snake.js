...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        this.cells = [new Cell(info.r, info.c)];  //存放蛇的身体cells[]存放蛇头

        this.next_cell = null;//下一步的目标位置
        this.speed = 5;         //蛇每秒钟走五个格子

        this.direction = -1;     //-1 表示没有指令，0，1，2，3表示上右下左
        this.status = "idle"     //idle表示闲置或静止，move表示正在移动，die 表示死亡
    ...
    next_step() {            //将蛇的状态变为走下一步
        ...
        if (!this.gamemap.check_vaild(this.next_cell)) {//下一步操作非法，蛇去世~~~~
            this.status = "die";
        }
    }
    update_move() {
        ...
        if (distance < this.eps) {    //走到目标点了
            ...
            if (!this.check_tail_increasing()) {//蛇尾不变长的情况:蛇尾跟着蛇头走到下一个目的地
                const k = this.cells.length;
